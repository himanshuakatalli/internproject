...
class Users extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('username, password, job_profile, organization', 'length', 'max'=>100),
			array('phone_number', 'length', 'max'=>25),
			array('hash, image', 'length', 'max'=>255),
			array('admin_notes, modify_date', 'safe'),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, first_name, last_name, username, password, phone_number, job_profile, organization, is_verified, hash, role_id, image, admin_notes, status, add_date, modify_date', 'safe', 'on'=>'search'),
		);
	}
